...
class asm6502():
    ...
    def strip_comments(self, thestring):
        ...
        if (position == -1):
            ...
            return (thestring[:position],thestring[position:])
    ...
    def strip_label(self, thestring, linenumber):
        ...
        if (position == -1):
            ...
            if (position == -1):
                ...
                self.warning(linenumber=linenumber,linetext=thestring,text="More than one thing in the label field. Ignoring everything between the first space and the colon")
    ...
    def identify_addressmode(self, opcode, premode, value, linenumber):
        ...
        self.debug(2,"INFO: GOT TO END OF IDENTIFY_ADDRESSMODE: Line %d opcode:%s premode:%s" % (linenumber, opcode,premode))
        return "UNDECIDED"
    ...
    def assemble(self,lines):
        ...
        for i in xrange(len(self.allstuff)):
            ...
            elif (lowbyte==-1) and ((addressmode in self.modeswithlowbytevalue) or (addressmode in self.modeswithhighbytevalue)):
